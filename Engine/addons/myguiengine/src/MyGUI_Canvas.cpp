...
namespace MyGUI
{
	...
	void Canvas::createTexture(IRenderTarget* _renderTarget)
	{
		destroyTexture();

		mTexture = RenderManager::getInstance().createTexture(mGenTexName);
		mTexture->setInvalidateListener(this);
		mTexture->createFromRenderTarget(_renderTarget);

		mTexResizeMode = TRM_PT_CONST_SIZE;
		if ( mReqTexSize.empty() )
			mReqTexSize = IntSize( mTexture->getWidth(), mTexture->getHeight() );
