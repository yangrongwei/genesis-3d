...
namespace MyGUI
{
	...
	void EditText::doRender()
	{
		...
		for (VectorLineInfo::const_iterator line = textViewData.begin(); line != textViewData.end(); ++line)
		{
			...
			for (VectorCharInfo::const_iterator sim = line->simbols.begin(); sim != line->simbols.end(); ++sim)
			{
				...
				if (select)
				{
					...
					if (!info)
					{
						mFont->getGlyphInfo(mBackgroundNormal ? FontCodeType::Selected : FontCodeType::SelectedBack, &info);	//[2012/5/8 zhongdaohuan]
					}
					drawGlyph(renderTargetInfo, vertex, vertexCount, vertexRect, info->uvRect, selectedColour);//[2012/5/8 zhongdaohuan]
				}
		...
		if (mVisibleCursor)
		{
			...
			GlyphInfo* cursorGlyph = nullptr;							//[2012/5/8 zhongdaohuan]
			mFont->getGlyphInfo(FontCodeType::Cursor, &cursorGlyph);	//[2012/5/8 zhongdaohuan]
