...
namespace MyGUI
{
	...
	void TextView::update(const UString& _text, IFont* _font, int _height, Align _align, VertexColourType _format, int _maxWidth)
	{
		...
		bool islock = false;	//[2012/5/8 zhongdaohuan]
...
Loop_Re:
			...
			GlyphInfo* info = nullptr;//[2012/5/8 zhongdaohuan]
			if(!_font->getGlyphInfo(character, &info))//[2012/5/8 zhongdaohuan]
			{
				if (!islock)
				{
					islock = _font->lock();
				...
				if (islock)
				{	
					GlyphInfo* new_info = nullptr;
					if(_font->appendChar(character, &new_info))
					{
						info = new_info;
					}
				}
			...
			if (FontCodeType::Space == character || (19968 <= character && character <= 40864))//[2012/4/27 zhongdaohuan] (19968~40864)中文文字域，中文文字是可以随时换行的。	
			{
			...
			if (_maxWidth != -1
				...
				&& !roll_back.empty())
			{
				//|| (19968 <= character && character <= 40864))//[2012/4/27 zhongdaohuan] (19968~40864)中文文字域，中文文字是可以随时换行的。		
				...
				// отменяем откат
				roll_back.clear();
				if (FontCodeType::Space != character && _text.begin() != index)//[2012/4/27 zhongdaohuan] 空格可以不画，非空格就不可以了。	
				{
					character = *index;
					goto Loop_Re;
				}
