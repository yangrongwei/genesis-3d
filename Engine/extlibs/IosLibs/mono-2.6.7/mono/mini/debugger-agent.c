...
suspend_init (void)
{
    ...
    char *name = g_strdup_printf ("/mono/%d/suspend", getpid() );
    sem_unlink( name );
    suspend_sem = sem_open( name, O_CREAT, 0644, 0);//sem_open( name, O_CREAT | O_EXCL, S_IRWXU, 0);
    if( suspend_sem == SEM_FAILED )
        abort();
}
