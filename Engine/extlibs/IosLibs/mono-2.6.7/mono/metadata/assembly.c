...
#include <glib.h>
#define MONO_CFG_DIR g_getenv ("MONO_CFG_DIR")

#define MONO_ASSEMBLIES *assemblies_path
...
fallback (void)
{
	mono_set_dirs (MONO_ASSEMBLIES, MONO_ASSEMBLIES);
}
...
set_dirs (char *exe)
{
	 ...
	 */
	if (strncmp (exe, MONO_ASSEMBLIES, strlen (MONO_ASSEMBLIES)) == 0 || (base = compute_base (exe)) == NULL){
		fallback ();
		return;
	}
