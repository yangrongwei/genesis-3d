...
void ManuallyCollection( float et )
{
    MGC_timer += et;
    if ( MGC_timer > 60.0f )
    {
        ...
        EXIT_GC();
        UNLOCK();
        Manually_mono_gc_finalize_notify();
    }
}
...
{
    if (!GC_incremental && !GC_dont_gc && !GC_manual_gc &&
