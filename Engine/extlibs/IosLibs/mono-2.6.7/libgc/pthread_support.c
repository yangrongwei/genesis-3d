...
#	endif
    ...
    int code = sem_post((si -> registered));	/* Last action on si.	*/
    					/* OK to deallocate.	*/
#   if defined(THREAD_LOCAL_ALLOC) && !defined(DBG_HDRS_ALL)
 	LOCK();
                  ...
                  void *(*start_routine)(void *), void *arg)
{
    ...
    char *name = g_strdup_printf ("/mono/pThread/%d", getpid ());
    sem_unlink( name );
    si -> registered = sem_open( name, O_CREAT, 0644, 0);//sem_open( name, O_CREAT | O_EXCL, S_IRWXU, 0);
    if( si -> registered == SEM_FAILED )
    {
        ABORT( g_strdup_printf( "sem_init_on_pthread_creat failed %d", errno ) );
    }
