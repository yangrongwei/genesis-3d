...
#define WRITER_EVENT_INIT() do {\
    ...
    */\
    profiler->enable_data_writer_event = sem_open( "sem_init_logging_1", O_CREAT, S_IRWXU, 0 );\
    profiler->wake_data_writer_event = sem_open( "sem_init_logging_2", O_CREAT, S_IRWXU, 0 );\
    profiler->done_data_writer_event = sem_open( "sem_init_logging_3", O_CREAT, S_IRWXU, 0 );\
    assert( false );\
} while (0)
#define WRITER_EVENT_DESTROY() do {\
	/*
    sem_destroy (&(profiler->enable_data_writer_event));\
	ç (&(profiler->wake_data_writer_event));\
	sem_destroy (&(profiler->done_data_writer_event));\
    */\
    sem_close( profiler->enable_data_writer_event );\
    sem_close( profiler->wake_data_writer_event );\
    sem_close( profiler->done_data_writer_event );\
    sem_unlink( "sem_init_logging_1" );\
    sem_unlink( "sem_init_logging_2" );\
    sem_unlink( "sem_init_logging_3" );\
    assert( false );\
} while (0)
