					   ...
					   guint32 exitstatus)
{
    ...
    sem_close( thread_handle->suspend_sem );
    sem_unlink( thread_handle->sem_name );
    free( thread_handle->sem_name);
...
static void _wapi_thread_suspend (struct _WapiHandle_thread *thread)
{
	...
	while ( 0 != ( code = sem_wait (thread->suspend_sem) ) &&
...
{
	int code = sem_post (thread->suspend_sem);
		      ...
		      gsize *tid) 
{
    ...
    char *name = g_strdup_printf ("/mono/wthreads/%d", getpid()+ ++sem_ws );
    sem_unlink( name );
    thread_handle_p->suspend_sem = sem_open( name, O_CREAT, 0644, 0);//sem_open( name, O_CREAT | O_EXCL, S_IRWXU, 0);
    if( thread_handle_p->suspend_sem == SEM_FAILED )
        abort();
    int  len = strlen( name );
    thread_handle_p->sem_name = malloc( len * sizeof(char) ) ;
    memcpy( thread_handle_p->sem_name, name, len);
