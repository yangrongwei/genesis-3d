...
void Manually_mono_gc_finalize_notify (void)
{
    sem_post( finalizer_sem );
}
...
finalizer_thread (gpointer unused)
{
	...
	while (!finished) {
        ...
        while ( 0 != ( code = sem_wait (finalizer_sem) ) );
#else
...
mono_gc_init (void)
{
    ...
    char *name = g_strdup_printf ("/mono/libGC/%d", getpid() );
    sem_unlink( name );
    finalizer_sem = sem_open( name, O_CREAT, 0644, 0);//sem_open( name, O_CREAT, S_IRWXU, 0);
    if( finalizer_sem == SEM_FAILED )
        abort();
#endif
