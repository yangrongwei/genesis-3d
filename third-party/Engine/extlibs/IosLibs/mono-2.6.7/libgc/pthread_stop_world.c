...
#   endif
    ...
    int code = sem_post(pGC_suspend_ack_sem);
    ...
    /* the only async-signal-safe primitive in LinuxThreads.    */
    code = sem_post(pGC_suspend_ack_sem);
...
#	  define RETRY_INTERVAL 100000
	  for (;;) {
	      ...
	      sem_getvalue(pGC_suspend_ack_sem, &ack_count);
...
#                 endif
    ...
    for (i = 0; i < n_live_threads; i++) {
	  while (0 != (code = sem_wait(pGC_suspend_ack_sem))) {
	      if (errno != EINTR) {
...
static void pthread_stop_init() {
    ...
    char *name = g_strdup_printf ("/mono/GC_Sem/%d", getpid ());
    sem_unlink( name );
    pGC_suspend_ack_sem = sem_open( name, O_CREAT, 0644, 0);//sem_open( name, O_CREAT | O_EXCL, S_IRWXU, 0);
    if( pGC_suspend_ack_sem == SEM_FAILED )
        ABORT( "Mono_GC_Sem failed" );
