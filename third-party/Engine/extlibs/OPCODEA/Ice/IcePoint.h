...
// Removed since it introduced the nasty "Point T = *Matrix4x4.GetTrans();" bug.......
		...
		inline_					Point(const float f[3]) : x(f[_XOP]), y(f[_YOP]), z(f[_ZOP])		{}
		//! Copy constructor
		...
		inline_	Point&			Set(const float f[3])					{ x  = f[_XOP];	y  = f[_YOP];	z  = f[_ZOP];			return *this;	}
		...
		//! Adds a vector
		inline_	Point&			Add(const float f[3])					{ x += f[_XOP];	y += f[_YOP];	z += f[_ZOP];			return *this;	}
		//! Adds vectors
		...
		//! Subtracts a vector
		inline_	Point&			Sub(const float f[3])					{ x -= f[_XOP];	y -= f[_YOP];	z -= f[_ZOP];			return *this;	}
		//! Subtracts vectors
		...
		inline_	PointComponent	LargestAxis()						const
								{
									const float* Vals = &x;
									PointComponent m = _XOP;
									if(Vals[_YOP] > Vals[m]) m = _YOP;
									if(Vals[_ZOP] > Vals[m]) m = _ZOP;
									return m;
								}
		...
		inline_	PointComponent	ClosestAxis()						const
								{
									const float* Vals = &x;
									PointComponent m = _XOP;
									if(AIR(Vals[_YOP]) > AIR(Vals[m])) m = _YOP;
									if(AIR(Vals[_ZOP]) > AIR(Vals[m])) m = _ZOP;
									return m;
								}
