...
上个语句会产生如下的链接错误，可能是因为调用的lseek函数是比较旧的函数，新的c运行时库已经没有该函数的实现了
5>libmpg123.lib(readers.obj) : error LNK2019: unresolved external symbol __imp__lseek referenced in function _posix_lseek
所以换成了以下的的调用*/
#ifdef __ANDROID__
static off_t   posix_lseek(int fd, off_t offset, int whence){ return lseek(fd, offset, whence); }
#else
